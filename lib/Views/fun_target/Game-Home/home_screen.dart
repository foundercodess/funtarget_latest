...
import 'package:funtarget/Constant/audio-player.dart';
import 'package:funtarget/Model/result-history-model.dart';
...
import 'package:funtarget/Views/fun_target/Game-Home/big_chakra.dart';
import 'package:funtarget/Views/fun_target/Game-Home/blinking.dart';
...
class _HomePageScreenState extends State<HomePageScreen> {
  ...
  void updateUI(Timer timer) {
    setState(() {
      if (countdownSeconds == 55) {
        ...
        pauseNetworkSound();
        playNetworkSound("https://kgfgold.in/assets/music/movechakra.mp3");
  ...
  hitInsertBetApiAndInsertBetData() async {
    ...
    await context.read<BetService>().InsertBetApi(jsonList).then((value) {
      setState(() {
        isBetOk = true;
      });
    }).catchError((err) {
    ...
    if (isBetOk) {
      ...
      for (BetOnNumbers bet in getBetOnNumber) {
        if (bet.number.toString() == result.toString()) {
          ...
          setState(() {
            winnerAmount = bet.betApplied * 10;
            needToTake = true;
            countdownTimer!.cancel();
            isViewResult = true;
            isBetOk = false;
          });
          Utils.toastMessage("YOU WON");
          foundWinner = true;
          break;
        }
      }
      if (!foundWinner) {
        ...
        Utils.errorToastMessage("YOU LOSE");
      }
    ...
    try {
      await Future.wait([
        Provider.of<ResultHistoryProvider>(context, listen: false)
            .fetchResulthistoryData(),
        Provider.of<ResultProvider>(context, listen: false).fetchResultData(),
    ...
    if (isBetOk == false) {
      setState(() {
        ...
        if (placeBetValue <= amount) {
          ...
          betNumbersList[index].betApplied =
          ...
          Utils.errorToastMessage("You have low balance");
        }
      });
    } else {
  ...
  Widget yellowButton(Widget? innerText, outerText, EdgeInsetsGeometry padding) {
    return Padding(
      padding: padding,
      child: Column(
        children: [
